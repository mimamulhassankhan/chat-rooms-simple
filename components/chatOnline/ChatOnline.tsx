...
const ChatOnline = ({ onlineUsers, setConversations }: any) => {
  const { token } = useCurrentUser();
  async function handleAvailableUserClick(receiverId: string, token: string) {
    try {
      const result = await axios.post(
        `http://localhost:8000/api/v1/create/${receiverId}`,
        {},
        {
          headers: {
      ...
      setConversations((prev: any) => [result.data.result, ...prev]);
    } catch (error) {
      console.log(error);
    }
  }
  return (
    <div className="chatOnline">
      {onlineUsers.map((f: any) => (
        <div
          className="chatOnlineFriend"
          onClick={() => handleAvailableUserClick(f.id, token)}
        >
