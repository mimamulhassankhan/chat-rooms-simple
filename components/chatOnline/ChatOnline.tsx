...
const ChatOnline = ({onlineUsers,currentUserId,setCurrentChat}:any) => {
    const [friends,setFriends]=useState([])
    const [onlineFriends,setOnlineFriends]=useState([])
    const getFriends=async()=>{
        try {
            const result=await axios.get(`http://localhost:8800/api/users/friends/${currentUserId}`)
    ...
    useEffect(()=>{
        const data=friends.filter((f:any)=>onlineUsers.some((u:any)=>u.userId ===f._id))
